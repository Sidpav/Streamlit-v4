...
def _excel_bytes(payload: dict):
    """
    Safe Excel exporter.
    Fixes openpyxl error: IndexError('At least one sheet must be visible')
    by ALWAYS writing at least one visible Summary sheet and replacing every empty sheet
    with a one-row placeholder dataframe.
    """
    ...
    def safe_value(value):
        """Make nested values Excel-cell friendly."""
        if value is None:
            return ""
    ...
    def safe_len(value):
        """Count rows/items safely for summary."""
        if value is None:
            return 0
    ...
    def to_df(data, fallback_message="No data available"):
        """Convert any result structure to a non-empty dataframe."""
        try:
            if data is None:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                if all(isinstance(x, dict) for x in data):
                    ...
                    df = pd.DataFrame({"Value": [safe_value(x) for x in data]})
                ...
                df = pd.DataFrame({"Value": [safe_value(data)]})

            if df is None or df.empty or len(df.columns) == 0:
                return pd.DataFrame([{"Message": fallback_message}])

            # Make nested cells JSON strings so openpyxl never fails on lists/dicts.
            ...
            return pd.DataFrame([{"Message": fallback_message, "Export Error": str(e)}])

    def clean_sheet_name(name):
        """Excel sheet names must be <=31 chars and cannot contain these characters."""
    ...
    def autosize(writer, sheet_name, df):
        """Make exported Excel easier to review."""
        try:
            ws = writer.sheets[sheet_name]
            for idx, col in enumerate(df.columns, 1):
                values = [str(col)] + [str(x) for x in df[col].astype(str).head(100).values]
    ...
    # Always create Summary first so at least one visible sheet exists.
    summary = pd.DataFrame([{
        "User Query": payload.get("user_query", ""),
        "Generated At": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "Total Gaps / Inconsistencies": safe_len(payload.get("inconsistencies", [])),
        "Total Novel Insights": safe_len(payload.get("novel_insights", [])),
        "Total Structured Facts": safe_len(payload.get("structured_facts", [])),
        "Total Retrieved Chunks": safe_len(payload.get("retrieved_chunks", [])),
        "Total Top-K / Experiment Results": safe_len(payload.get("all_results", [])),
    }])

    sheets = {
        "Summary": summary,
        "Gaps Inconsistencies": to_df(
            payload.get("inconsistencies", []),
            "No gaps or inconsistencies detected"
        ),
        "Novel Insights": to_df(
            payload.get("novel_insights", []),
            "No novel insights detected"
        ),
        "Structured Facts": to_df(
            payload.get("structured_facts", []),
            "No structured facts extracted"
        ),
        "Retrieved Chunks": to_df(
            payload.get("retrieved_chunks", []),
            "No retrieved chunks available"
        ),
        "TopK Results": to_df(
            payload.get("all_results", []),
            "No Top-K experiment results available"
        ),
        "Raw Payload": to_df(
            payload,
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for sheet_name, df in sheets.items():
            safe_sheet = clean_sheet_name(sheet_name)

            if df is None or df.empty or len(df.columns) == 0:
                df = pd.DataFrame([{"Message": "No data available"}])

            df.to_excel(writer, index=False, sheet_name=safe_sheet)
            autosize(writer, safe_sheet, df)
            written_sheet_count += 1

        # Absolute safety fallback. This should never trigger, but prevents openpyxl crash.
        if written_sheet_count == 0:
            pd.DataFrame([{"Message": "No export data available"}]).to_excel(
